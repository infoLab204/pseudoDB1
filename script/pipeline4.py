...
def pre_align(species, reference_file) :
    ...
    if not missing_files :
        ...
        os.system(f"gatk CreateSequenceDictionary -R {home_path}/{species}/data/ref/{reference_file}")
...
def pseudo_db(species, reference_file):
    ...
    for i in range(len(sample)) : 
        cmd_line=f"gatk HaplotypeCaller -R {home_path}/{species}/data/ref/{reference_file} "
        cmd_line=cmd_line+f" -I  {home_path}/{species}/module/align/{sample[i]} "
        cmd_line=cmd_line+f" -O {home_path}/{species}/data/db/{sample[i][:sample[i].find('_aligned.bam')]}.g.vcf.gz -ERC GVCF "
        print(cmd_line)
        os.system(cmd_line)
        
    cmd_line=""
    for i in range(len(sample)) :  
        cmd_line=cmd_line + f"-V {home_path}/{species}/data/db/{sample[i][:sample[i].find('_aligned.bam')]}.g.vcf.gz "
    cmd_line=cmd_line + f"-O {home_path}/{species}/data/db/{species}_pseudoDB_g.vcf.gz "
    
    os.system(f"gatk CombineGVCFs -R {home_path}/{species}/data/ref/{reference_file} {cmd_line}")
    
    # delete file
    for i in range(len(sample)) : 
        os.system(f"rm -rf {home_path}/{species}/module/db/{sample[i][:sample[i].find('_aligned.bam')]}.g.vcf.gz \
                {home_path}/{species}/module/db/{sample[i][:sample[i].find('_aligned.bam')]}.g.vcf.gz.tbi") 

    # GenotypeGVCFs
    os.system(f"gatk GenotypeGVCFs -R {home_path}/{species}/data/ref/{reference_file} \
            -V  {home_path}/{species}/data/db/{species}_pseudoDB_g.vcf.gz \
            -O  {home_path}/{species}/data/db/{species}_pseudoDB.vcf.gz ")
	
    # delete file
    os.system(f"rm -rf {home_path}/{species}/data/db/{species}_pseudoDB_g.vcf.gz \
            {home_path}/{species}/data/db/{species}_pseudoDB_g.vcf.gz.tbi ")
...
def qs_recal(species, reference_file, dbtype,sample_name) : 
    ...
    database_idx=f"{species}_{dbtype}.vcf.gz.tbi"
    if database not in vcf_list :
        sys.exit("Not found database")
   
    # run each sample
    # BaseRecalibrator 
    os.system(f"gatk BaseRecalibrator -R {home_path}/{species}/data/ref/{reference_file} \
            -I {home_path}/{species}/module/align/{sample_name}_aligned.bam  \
            --known-sites {home_path}/{species}/data/db/{database} \
    ...
    # ApplyBQSR
    os.system(f"gatk ApplyBQSR -R {home_path}/{species}/data/ref/{reference_file} \
            -I {home_path}/{species}/module/align/{sample_name}_aligned.bam   \
            --bqsr-recal-file {home_path}/{species}/module/machine/{sample_name}_{dbtype}_recalibration_table \
...
def variant_call(species, reference_file, dbtype):
    ...
    # HaplotypeCaller 
    for i in range(len(sample)) :
        os.system(f"gatk HaplotypeCaller -R {home_path}/{species}/data/ref/{reference_file} \
                -I  {home_path}/{species}/module/machine/{sample[i]}_{dbtype}_recalibrated.bam \
                -O  {home_path}/{species}/module/variants/{sample[i]}_{dbtype}.g.vcf.gz -ERC GVCF \
                 &> {home_path}/{species}/module/variants/{sample[i]}_{dbtype}.g.vcf.gz.log")


    # Call genetic variants - step2 : Joint-Call Cohort 
    path_dir=f"{home_path}/{species}/module/variants"
    file_list=os.listdir(path_dir)

    sample=[]
    type_name=f"_{dbtype}.g.vcf.gz"
    for file_name in file_list  :
        if file_name.find(type_name) !=-1 :
            sample.append(file_name[:file_name.find(type_name)])
    ...
    sample_list=""
    for i in range(len(sample)) :  
        sample_list=sample_list + f"-V {home_path}/{species}/module/variants/{sample[i]}_{dbtype}.g.vcf.gz "
    sample_list=sample_list + f"-O {home_path}/{species}/module/variants/{species}_{dbtype}.g.cohort.vcf.gz \
            &> {home_path}/{species}/module/variants/{species}_{dbtype}.g.cohort.vcf.gz.log"

    os.system(f"java -jar {home_path}{GATK4} CombineGVCFs -R {home_path}/{species}/data/ref/{reference_file} {sample_list}")
    
    # delete file
    for i in range(len(sample)) : 
        os.system(f"rm -rf {home_path}/{species}/module/variants/{sample[i]}_{dbtype}.g.vcf.gz \
            {home_path}/{species}/module/variants/{sample[i]}_{dbtype}.g.vcf.gz.tbi \
            {home_path}/{species}/module/variants/{sample[i]}_{dbtype}.g.vcf.gz.log")

    # GenotypeGVCFs
    os.system(f"gatk GenotypeGVCFs \
            -R {home_path}/{species}/data/ref/{reference_file} \
            -V  {home_path}/{species}/module/variants/{species}_{dbtype}.g.cohort.vcf.gz \
            -O  {home_path}/{species}/module/variants/{species}_{dbtype}_variant_calling.vcf.gz  \
             &> {home_path}/{species}/module/variants/{species}_{dbtype}_variant_calling.vcf.gz.log ")
	
    # delete file
    os.system(f"rm -rf {home_path}/{species}/module/variants/{species}_{dbtype}.g.cohort.vcf.gz \
            {home_path}/{species}/module/variants/{species}_{dbtype}.g.cohort.vcf.gz.tbi \
            {home_path}/{species}/module/variants/{species}_{dbtype}.g.cohort.vcf.gz.log \
            {home_path}/{species}/module/variants/{species}_{dbtype}_variant_calling.vcf.gz.log")
